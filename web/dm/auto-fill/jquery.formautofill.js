			...
			return this.each(function() {
				$.each( data, function(k, v) {
					...
					if ( settings.findbyname ) { // by name
						...
						if ( elt.length == 1 ) {
							//add by evan,只读text字段为空显示---
							if(elt.attr("class") && elt.attr("class").indexOf("display-text")>=0){								
								if(elt.attr("istime")){
									if(elt.is("input")){
										elt.val(window.format.getTimeValue(null,null,v));
									}else{
										elt.html(window.format.getTimeValue(null,null,v));
									}
								}else{
									if(elt.is("input")){
										elt.val(window.format.getDisplayValue(null,null,v));
									}else{
										elt.html(window.format.getDisplayValue(null,null,v));
									}
								}								
							}else{
