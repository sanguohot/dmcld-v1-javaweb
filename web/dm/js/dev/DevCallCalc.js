define(["dev-sch",'dev-fun','dev-call-fun','dev-call-chart','dev-call-list'
        ,'text!html/dev/NoData.html',"text!html/DateTimeRow2.html"],function (sch,fun,cfun,chart,list,ndTpl,datetime){	
	function getHtml(pid){
		...
		var fid=pid+"_form";
		var lpid=pid+"_plist";
		var sn=window.devCall.productSn;
		var tmp=sn?(' ['+window.devCall.productSn+']'):"";
		...
		html+='<h5 class="header smaller lighter red">'+window.lan["callCalc"]+tmp
		html+='<div style="float:right;" class="btn-group">';
		if(pid!="dev_call_calc")
...
//		,{text:window.lc.getValue("lastDay"),value:"day"},{text:window.lc.getValue("lastWeek"),value:"week"}]);
	...
	function exchange(pid){
		if(window.devCall.view=="chart"){
			window.devCall.view="list";
		}else if(window.devCall.view=="list"){
			window.devCall.view="chart";
		}
		refresh(pid);
	}
...
//			pickerPosition:'bottom-left',
	...
	function createCallCalc(pid,neUuid,productSn){
		window.devCall={productSn:productSn,neUuid:neUuid,view:'chart'};
		createView(pid);
		listen(pid);
	...
	function loadRemoteData(pid){
		...
		$("#"+cid).html(fun.getLoadHtml());
		 $.ajax({ url: "androidManager!getCallCalc.action?totalDot=1", data:params,complete: function(data,str){
			if(data.responseJSON && data.responseJSON.success){
				var obj=data.responseJSON.dmCall;
				//缓存远端设备统计数据
				if(obj){
					window.devCall.data=obj;
				}else{
					window.devCall.data={};
				}
