define(["dev-sch",'dev-fun','dev-call-fun','user-report-charts','user-report-list',
	...
	function getHtml(pid){
		...
		html+='<h5 class="header smaller lighter red">'+window.lan["userReport"]
...
//		,{text:window.lc.getValue("lastDay"),value:"day"},{text:window.lc.getValue("lastWeek"),value:"week"}]);
	...
	function exchange(pid){
		if(window.userReport.view=="chart"){
			window.userReport.view="list";
		}else if(window.userReport.view=="list"){
			window.userReport.view="chart";
		}
		refresh(pid);
	}
...
//			pickerPosition:'bottom-left',
	...
	function loadRemoteData(pid){
		//var params={productSn:window.userReport.productSn,neUuid:window.userReport.neUuid};
		...
		$("#"+cid).html(fun.getLoadHtml());
		 $.ajax({ 
			...
			if(data.responseJSON && data.responseJSON.success){
				...
				if(obj  && obj.tbluserReportList.length){
					for(var i=0;i<obj.tbluserReportList.length;i++){
						var item=obj.tbluserReportList[i];
