define(["dev-sch",'dev-fun','dev-call-fun','alarm-report-charts','alarm-report-list'
	...
	function getHtml(pid){
		...
		html+='<h5 class="header smaller lighter red">'+window.lan["alarmReport"]
...
//		,{text:window.lc.getValue("lastDay"),value:"day"},{text:window.lc.getValue("lastWeek"),value:"week"}]);
	...
	function exchange(pid){
		if(window.alarmReport.view=="chart"){
			window.alarmReport.view="list";
		}else if(window.alarmReport.view=="list"){
			window.alarmReport.view="chart";
		}
		refresh(pid);
	}
...
//			pickerPosition:'bottom-left',
	...
	function createAlarmReport(pid){
		window.alarmReport={view:'chart'};
		createView(pid);
		listen(pid);
	...
	function loadRemoteData(pid){
		//var params={productSn:window.alarmReport.productSn,neUuid:window.alarmReport.neUuid};
		...
		$("#"+cid).html(fun.getLoadHtml());
		 $.ajax({ 
			 url: "reportManager!getAlarmReportList.action", 
			 data:params,
			...
			if(data.responseJSON && data.responseJSON.success){
				...
				if(obj  && obj.tblAlarmReportList.length){
					for(var i=0;i<obj.tblAlarmReportList.length;i++){
						var item=obj.tblAlarmReportList[i];
    ...
    return {
		...
		getHtml:getHtml,
		createAlarmReport:createAlarmReport
    };
});