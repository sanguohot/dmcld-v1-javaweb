...
//			height: 300,
			...
			columns: [{
				...
				title: window.lc.getValue("diskUsed"),
				align: 'center',
				valign: 'middle',
				sortable: true,
			}],
	...
	function createChart(pid,id,obj,title,nl,tl){
		var oa=[];
...
//		+'</div>'
		...
		+'<div class="row " style="padding:0">'
			+'<div class="col-md-12" style="padding:0">'
				+'<div id="'+cid+'_3'+'" style="height:300px;width:100%;margin-top:10px;"></div>'
			+'</div>'
			+'<div class="col-md-12" style="padding:0">'
				+'<div id="'+cid+'_2'+'" style="height:300px;width:100%;margin-top:26px;"></div>'
			+'</div>'
			+'<div class="col-md-12" style="padding:0">'
				+'<div id="'+cid+'_1'+'" style="height:300px;width:100%;margin-top:26px;"></div>'
			+'</div>'
		+'</div>'		
		+'</div>'		
//		+'<div class="container-fluid my-tab" >'
...
//		+'</div>'
	...
	function refreshCalc(pid,obj){
		...
		var fid=pid+"_form";
		var lpid=pid+"_plist";
		var nl1=["avgProcessTime","maxProcessTime"],tl1=[window.lc.getValue("avgProcessTime")+"(ms)",window.lc.getValue("maxProcessTime")+"(ms)"];
		createChart(pid,cid+"_1",obj,window.lc.getValue("processTime")+"(ms)",nl1,tl1);
		var nl1=["curPendingCnt","totalProcess","curDealCnt"]
		         ,tl1=[window.lc.getValue("curPendingCnt"),window.lc.getValue("totalProcess"),window.lc.getValue("curDealCnt")];
		createChart(pid,cid+"_2",obj,window.lc.getValue("processCnt"),nl1,tl1);
		var nl1=["curDealStatus1","curDealStatus2","curDealStatus3","curDealStatus4"]
		         ,tl1=[window.lc.getValue("curDealStatus",1),window.lc.getValue("curDealStatus",2)
		               ,window.lc.getValue("curDealStatus",3),window.lc.getValue("curDealStatus",4)];
		createChart(pid,cid+"_3",obj,"11111",nl1,tl1);
		var n=$("#"+lpid);
		n.html("");
...
//			$('#'+lid).bootstrapTable("refresh");
	...
	function loadRemoteData(pid){
		...
		params["dstDomainUuid"]=domainUuid;
		 $.ajax({ url: "dmManager!getDcHis.action", data:params,complete: function(data,str){
			if(data.responseJSON && data.responseJSON.success){
				var obj=data.responseJSON.dcList;
