...
define([],function(){
  ...
  function beginSch(){
      ...
      var activeId=node.attr("href");
		...
		if(pa.indexOf("unknownDev")>=0 && !$("#dev_unknown_list").length){
			...
			window.devMain.procTab();
		}else if(pa.indexOf("unknownDev")<0 && $("#dev_unknown_list").length){
			window.devMain.createHtml("my-tab-position");
			window.devMain.procTab();
		}
      window.devMain.tabAfterShow(activeId);
  }
  function getSelSearch(type,content){
    var a='<a class="param-selected myicon-tag J_Ajax" href="#" data-url="nav"'
  ...
  function getSelSearchs(){
    ...
    m.each(function(key,value,index){
      ...
      if(content!=""){
        html=html+getSelSearch(type,content);
      }
    });
    return html;
  }
  ...
  function createSelSearch(type,content){
    var a=getSelSearch(type,content);
  ...
  function getSchHtml(devObj){
    ...
    var html= '<div class="bread-crumbs row J_breadcrumbs"><div class="bread-crumbs">'
      +'<div class="counts">'
      +'<span class="crumbs-items">'+window.lc.getValue("total")+' <span class="H">'
      +devObj.total+'</span> '+window.lc.getValue("devs")+'</span>'
    ...
    if(devObj.children.length){
      ...
      for(var i=0;i<devObj.children.length;i++){
        ...
        tmp=tmp+'<div class="nav-block J_commonBlock type-text">'
...
//	$(".J_navSwitchBtn").trigger("click");
  ...
  function loadSearch(){
    ...
    var deviceObj={name:"allClassify",total:5000,pobj:null,pname:"",children:[]};
...
//	deviceStatus.children.push(record);
    ...
    deviceStatus.children.push(record);
    var record={name:"disable",pname:"disable",children:[]};
    deviceStatus.children.push(record);
