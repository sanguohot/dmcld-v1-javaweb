...
Ext.define('Ext.ux.desktop.Desktop', {
    ...
    createWindow: function(config, cls) {
        ...
        win.doClose = function ()  {
            ...
            if(!ip.closeAllFlag){
            	Ext.suspendLayouts();
            	ip.closeAllFlag=true;
            	var tb=Ext.getCmp('mp').down('toolbar').query('button');
            	...
            	for(var i=0;i<me.windows.getCount();i++){
            		var twin=me.windows.getAt(i);
            		console.log('twin='+twin.id);
            		if(win.id!=twin.id){
            			if(twin.id!='operation_win' && twin.id!='maintenance_win'&& twin.id!='monitor_win'){
            				twin.close();
            			}else{
            				twin.minimize();
            			}
    ...
    initShortcut : function() { 
        ...
        for (var i = 0, len = items.length; i < len; i++) {    
            ...
            if(numberOfItems==0 || i==0){
            	Ext.fly(items[i]).setXY([col.x, row.y-20]);    
            	row.index++;    
            	row.y = row.y + btnHeight + btnPadding-20;    
            }else{
