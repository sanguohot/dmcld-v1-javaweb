Ext.define('app.view.license.LicPaidListTab',{
...
//			if(Ext.get('username').value=='finance' || Ext.get('username').value=='root'){
				var add = Ext.create('Ext.button.Button',{
		       		 ...
		       		 listeners:{
							...
							var addLicPaidCard = Ext.getCmp('addLicPaidCard');
							if(addLicPaidCard=='undefined'||addLicPaidCard==undefined){
								addLicPaidCard=Ext.create('app.view.license.AddLicPaidCard');
								lanControll.setLan(addLicPaidCard);
							}
							addLicPaidCard.cmpId=paidListGrid.up('panel').id;
							addLicPaidCard.show();
						}
		       	 	}
		       	 });
					...
					click:function(){
						if(paidListGrid.getSelectionModel().hasSelection()){
	       		 			...
	       		 			for(var i=0;i<records.length;i++){
	       		 				if(records[i].get('cardStatus')==0 || records[i].get('cardStatus')==2){
	       		 					continue;
	       		 				}
		       		 		...
		       		 		var updateLicPaidCard = Ext.getCmp('updateLicPaidCard');
							if(updateLicPaidCard=='undefined'||updateLicPaidCard==undefined){
								updateLicPaidCard=Ext.create('app.view.license.UpdateLicPaidCard');
								lanControll.setLan(updateLicPaidCard);
							}
							updateLicPaidCard.down('form').getForm().findField('uuids').setValue(ids);
							updateLicPaidCard.show();
	   		 			}else{
				...
				var importS = Ext.create('Ext.button.Button',{
		       		 ...
		       		 listeners:{
		       		 	click:function(){
							var cmpId=paidListGrid.up('panel').up('panel').id;
		       		 		var importConfig=Ext.getCmp('importConfig');
		       		 		if(!importConfig){
		       		 			importConfig=Ext.create('app.view.operation.ImportConfig',{importMode:'importLicPaidCard',cmpId:cmpId});
		       		 			lanControll.setLan(importConfig);
		       		 		}
		       		 		importConfig.importMode="importLicPaidCard";
