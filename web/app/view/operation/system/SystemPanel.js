...
Ext.define('app.view.operation.system.SystemPanel',{
...
//		store.proxy.url = "sysManager!getSysListWithDeviceNum.action";
...
//				},
		...
		var id = 'domainInSysPanel';
		if(maintenance){
			id = 'maintenanceDomainInSysPanel';
		}
		...
		var id = 'nesInSystemTab';
		if(maintenance){
			id = 'maintenanceNesInSystemTab';
		}
		...
		var id = 'neNasInSystemTab';
		if(maintenance){
			id = 'maintenanceNeNasInSystemTab';
		}
...
//			  var licNewSignType=parseInt(r.get('licNewSignType'));
//			  var licNewHbmFlag=parseInt(r.get('licNewHbmFlag'));
...
//			  var licNewSignTypeField=systemTab1.getForm().findField('licNewSignType');
//			  var licNewHbmFlagField=systemTab1.getForm().findField('licNewHbmFlag');
			  Ext.suspendLayouts();	
...
//			  licNewHbmFlagField.setValue(rs.yesOrNo(licNewHbmFlag));
		...
		var manDomainUuid = systemTab1.down('combo[name=manageDomain]');
		var manDomainStore = manDomainUuid.store;
		comboxStore.on('load',function(){
			manDomainStore.removeAll();
			manDomainStore.add({uuid:0,name:'NULL'});
...
//			store.load();
		...
		this.items=[{
	       	xtype: 'tabpanel',
//	       	id:'cloudTab',
	       	items:[systemTab1,systemTab2,systemTab3],
	       	listeners:{			
