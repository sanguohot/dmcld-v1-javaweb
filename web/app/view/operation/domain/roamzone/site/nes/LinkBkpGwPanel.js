Ext.define('app.view.operation.domain.roamzone.site.nes.LinkBkpGwPanel',{
		            ...
		            listeners:{
		                render : function(p) {
		                    p.getEl().on('mouseup', function(p){ 
		                    	var tip = Ext.getCmp('LinkBkpGwPanel_tip');
		                    	tip.show();
		                    });
	            		},
		        		focus:function(){
		        			...
		        			if(tip==undefined || tip==null){
		        				...
		        				tip = tipManage.createObjNameTipObj('LinkBkpGwPanel_tip',textobj.getEl().dom.id, gettip.getObjNameTip(textobj.fieldLabel));
		        			}
		        			tip.show();
		        			tip.clearListeners();
		        			//alert(tmp.html)
...
//						statusId : 3
	            ...
	            fieldLabel: 'Run Status',
				   ...
				   items: [
							...
							{xtype: 'button',ulan:'btToDwgPort',text:'>>To DWG Port',id:'linkDwgGwPort',
								
								listeners:{
									click:function(but,even,eOpts){
					        			
								var sgs=Ext.create('app.util.SetGwpStore',{});
								var linkBkpGwPanel=Ext.getCmp('linkBkpGwPanel');
								var uuid=bkpInfoTab.getForm().findField('gwpUuid').getValue();
								var alias="";
								var gwpInfoTab=linkBkpGwPanel.up('panel').up('panel').items.get(3);
								var isOpen=false;
								
								for(var i=0;i<10;i++){
									gwpInfoTab=linkBkpGwPanel.up('panel').up('panel').items.get(i);
								...
								if(!isOpen){
									var linkGwpPanel=Ext.getCmp('linkGwpGwPanel');
					 				
					 				if(linkGwpPanel==undefined || linkGwpPanel=='undefined'){
					 					
					 					linkGwpPanel=Ext.create('app.view.operation.domain.roamzone.site.nes.LinkGwpGwPanel',{});
					 				}
					 			
					 				linkGwpPanel.setVisible(true);
					 				var linkBkpGwPort=Ext.getCmp('linkBkpGwPort');
					 				gwpInfoTab=sgs.setGwpStoreAndCreateTab(uuid,linkGwpPanel,linkBkpGwPanel.up('panel').up('panel'),linkBkpGwPort);
				 					...
				 					var linkBkpGwPort=Ext.getCmp('linkBkpGwPort');
				 					sgs.setGwpStore(uuid,linkGwpPanel,gwpInfoTab,linkBkpGwPort);
				        			gwpInfoTab.show();
				 			}
					        		}
				   ...
				   items: [
							...
							{xtype: 'button',ulan:'btToSimCard',text:'>>To SIM Card',id:'linkSimCardGw',
								...
								listeners:{
									click:function(but,even,eOpts){
					        			...
					        			if(simUuid<1){
					        				...
					        				var simCardInfoTab=linkBkpGwPanel.up('panel').up('panel').items.get(2);
					        				var isOpen=false;
					        				
					        				for(var i=0;i<10;i++){
					        					simCardInfoTab=linkBkpGwPanel.up('panel').up('panel').items.get(i);
						        			...
						        			if(!isOpen){
						        				...
						        				if(!simCardPanel){
						        					simCardPanel=Ext.create('app.view.operation.domain.group.SimCardGwPanel',{});
						        				}
						        				ssc.setSimCardStoreAndCreateTab(simUuid,domainUuid,simCardPanel,linkBkpGwPanel.up('panel').up('panel'),Ext.getCmp('toSimBankGwPort'));
						        			}else{
						        				var simCardPanel=Ext.getCmp('simCardGwPanel');
							        			...
							        			ssc.setSimCardStore(simUuid,domainUuid,simCardPanel,simCardInfoTab,Ext.getCmp('toSimBankGwPort'));
						        			}
...
//			                    		Ext.getCmp('operationTree').store=s;
		       		...
		       		ulan:'btRefresh',
		       		 ...
		       		 listeners:{
		        			...
		        			var uuid=Ext.getCmp('linkBkpGwTab').getForm().findField('uuid').getValue();
		        			var bkpInfoPanel=Ext.getCmp('linkBkpGwPanel');
		        			var bkpInfoTab=bkpInfoPanel.up('panel');
		        			var toDwgPort=Ext.getCmp('linkDwgGwPort');
		        			var toSimCard=Ext.getCmp('linkSimCardGw');
		        			uuid=parseInt(uuid);
