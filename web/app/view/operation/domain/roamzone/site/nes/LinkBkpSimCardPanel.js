Ext.define('app.view.operation.domain.roamzone.site.nes.LinkBkpSimCardPanel',{
	extend:'Ext.panel.Panel',
	id:'linkBkpSimCardPanel',
	requires: [
		        'Ext.util.Format',
		        'Ext.grid.Panel',
		       ],
		            ...
		            listeners:{
		                render : function(p) {
		                    p.getEl().on('mouseup', function(p){ 
		                    	var tip = Ext.getCmp('LinkBkpSimCardPanel_tip');
		                    	tip.show();
		                    });
	                	},
		        		focus:function(){
		        			...
		        			if(tip==undefined || tip==null){
		        				...
		        				tip = tipManage.createObjNameTipObj('LinkBkpSimCardPanel_tip',textobj.getEl().dom.id, gettip.getObjNameTip(textobj.fieldLabel));
		        			}
		        			tip.show();
		        			tip.clearListeners();
		        			//alert(tmp.html)
...
//						statusId : 3
	            ...
	            fieldLabel: 'Run Status',
				   ...
				   items: [
							...
							{xtype: 'button',ulan:'btToDwgPort',text:'>>To DWG Port',id:'linkDwgSimCardPort',
								
								listeners:{
									click:function(but,even,eOpts){
					        			
										var sgs=Ext.create('app.util.SetGwpStore',{});
										var linkBkpPanel=Ext.getCmp('linkBkpSimCardPanel');
										...
										if(!isOpen){
											var linkGwpPanel=Ext.getCmp('linkGwpSimCardPanel');
							 				...
							 				if(linkGwpPanel==undefined || linkGwpPanel=='undefined'){
							 					...
							 					linkGwpPanel=Ext.create('app.view.operation.domain.roamzone.site.nes.LinkGwpSimCardPanel',{});
							 				}
							 				linkGwpPanel.setVisible(true);
							 				var linkBkpPort=Ext.getCmp('linkBkpSimCardPort');
				   ...
				   items: [
							...
							{xtype: 'button',ulan:'btToSimCard',text:'>>To SIM Card',id:'linkSimCardSimCard',
								...
								listeners:{
									click:function(but,even,eOpts){
					        			...
					        			if(simUuid<1){
						        			...
						        			if(!simCardInfoTab){
						        				simCardInfoTab=Ext.getCmp('simCardTab');
						        			}
						        			if(!isOpen){
						        				
						        				var simCardPanel=Ext.getCmp('simCardInfoPanel');
						        				if(!simCardPanel){
						        					simCardPanel=Ext.create('app.view.operation.domain.group.SimCardPanel',{});
						        				}
						        				ssc.setSimCardStoreAndCreateTab(simUuid,domainUuid,simCardPanel,linkBkpPanel.up('panel').up('panel'),Ext.getCmp('toSimBankPort'));
						        			}else{
						        				var simCardPanel=Ext.getCmp('simCardInfoPanel');
							        			if(simCardPanel){
							        			...
							        			ssc.setSimCardStore(simUuid,domainUuid,simCardPanel,simCardInfoTab,Ext.getCmp('toSimBankPort'));
						        			}
...
//			                    		Ext.getCmp('operationTree').store=s;
		       		...
		       		ulan:'btRefresh',
		       		 ...
		       		 listeners:{
		        			...
		        			var uuid=Ext.getCmp('linkBkpSimCardTab').getForm().findField('uuid').getValue();
		        			var bkpInfoPanel=Ext.getCmp('linkBkpSimCardPanel');
		        			var bkpInfoTab=bkpInfoPanel.up('panel');
		        			var toDwgPort=Ext.getCmp('linkDwgSimCardPort');
		        			var toSimCard=Ext.getCmp('linkSimCardSimCard');
		        			uuid=parseInt(uuid);
