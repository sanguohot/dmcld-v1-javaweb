Ext.define('app.view.operation.domain.roamzone.site.nes.LinkGwpGwPanel',{
		            ...
		            listeners:{
		                render : function(p) {
		                    p.getEl().on('mouseup', function(p){ 
		                    	var tip = Ext.getCmp('LinkGwpGwPanel_tip');
		                    	tip.show();
		                    });
	                	},
		        		focus:function(){
		        			...
		        			if(tip==undefined || tip==null){
		        				...
		        				tip = tipManage.createObjNameTipObj('LinkGwpGwPanel_tip',textobj.getEl().dom.id, gettip.getObjNameTip(textobj.fieldLabel));
		        			}
		        			tip.show();
		        			tip.clearListeners();
		        			//alert(tmp.html)
...
//						statusId : 3
			   ...
			   items: [
						...
						{xtype: 'button',ulan:'btToSimbankPort',text:' >> To SIMBANK Port',id:'linkBkpGwPort',
							listeners:{
							...
							var ss=Ext.create('app.util.SetBkpStore',{});
			     			...
			     			if(!isOpen){
			     				...
			     				ss.setBkpStoreAndCreateTab(uuid,bkpInfoPanel,linkGwpPanel.up('panel').up('panel'),linkDwgGwPort,linkSimCardGw);
		     					...
		     					var linkDwgGwPort=Ext.getCmp('linkDwgGwPort');
			     				var linkSimCardGw=Ext.getCmp('linkSimCardGw');
		     					ss.setBkpStore(uuid,bkpInfoPanel,bkpInfoTab,linkDwgGwPort,linkSimCardGw);
...
//			                    		Ext.getCmp('operationTree').store=s;
		       		...
		       		ulan:'btRefresh',
		       		 ...
		       		 listeners:{
		        			...
		        			var uuid=Ext.getCmp('linkGwpGwTab').getForm().findField('uuid').getValue();
		        			var linkGwpGwPanel=Ext.getCmp('linkGwpGwPanel');
		        			var gwpInfoTab=linkGwpGwPanel.up('panel');
		        			var toBkpPort=Ext.getCmp('linkBkpGwPort');
		        			uuid=parseInt(uuid);
