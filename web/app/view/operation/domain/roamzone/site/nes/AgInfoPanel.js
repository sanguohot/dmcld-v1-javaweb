Ext.define('app.view.operation.domain.roamzone.site.nes.AgInfoPanel',{
...
//	comboxStore:Ext.create("app.store.util.ComboxStore",{}),
	...
	initComponent: function(){
		var store = Ext.create('app.store.operation.domain.roamzone.site.nes.AgInfoStore',{});
		var agAlias;
		this.store=store;
		...
		if(!maintenance){
			...
			this.agpListToolbars=3;
			this.agpListOtiose=0;
			this.agpMapOtiose=0;
			this.agpMapToolbars=3;
			var generalObj = Ext.getCmp('GeneralObj');
			if(!generalObj){
				generalObj = Ext.create('app.util.GeneralObj',{});
			}
			agAlias = generalObj.createName('ag_alias'
					,75,25,'alias','Alias','#DFE9F6','neManager!checkAlias.action',store);
			store.on('load',function(){
    			var picture = agAlias.getComponent('picture');
    			picture.update("");
    			picture.flag = 2;
			});
		}else{
			...
			this.agpMapToolbars=2;
			this.agpMapOtiose=1;
			this.agpListToolbars=2;
			this.agpListOtiose=1;
			this.syslogToolbars=2;
			agAlias = Ext.create('Ext.form.field.Text',{
	    		xtype: 'textfield',
				name : 'alias',
				fieldLabel: 'Alias',
				labelWidth: 180,
		...
		var agTab=Ext.create('Ext.form.Panel',{
			title:lanControll.getLanValue('tiDagInfo'),
			border:false,
			autoScroll:true,
			forceRefresh:0,
			store:store,
	        ...
	        items: [{
				...
				items:[{
			        ...
			        items: [{
			    		...
			    		src:Ext.get('resources').value+'/images/panel_logo/ag.png',
...
//		        },
		        {
		        	...
		        	xtype: 'displayfield',
		        	name: 'storeUsage',
		        	fieldLabel: 'Store Usage',
		        	fieldCls:'storeUsageCls'
		        },{
		        	xtype: 'displayfield',
		        	name: 'memAosUsage',
		        	fieldLabel: 'Mem AOS Usage',
		        	fieldCls:'memAosUsageCls'
		        },{
		        	xtype: 'displayfield',
		        	name: 'memLinuxUsage',
		        	fieldLabel: 'memLinuxUsage',
		        	fieldCls:'memLinuxUsageCls'
		        }]
			},{
	            xtype: 'fieldset',
				layout:'anchor',
		...
		agTab.addListener("afterlayout",function(){
			privilege.procPrivilege(agTab);
		...
		var commit={
	            ...
	            disabled: true,
	            handler: function() {
	        		var tmp = this.up('form').down('fieldcontainer[itemId=ag_alias]');
	        		if(tmp.getComponent('picture').flag==0)
	            	return;
	                var form = this.up('form').getForm();
	                if (form.isValid()) {
	                	Ext.Ajax.request({
	                		url:'agManager!updateAg.action',
	                		method:'POST',
	                		params:form.getValues(),
	                		callback: function (options, success, response) {
		                    	var obj=Ext.JSON.decode(response.responseText);			
		                    	if(obj['success']){
		                    		ip.commitSuccess(agTab,agTab.store);
		                    	}else{
		                    		ip.commitFailure(agTab);
		                    	}
	                    	}
	                	});
	                }
...
//	       			ulan:'miNewTab',
//	       			handler:function(){
//       		 			var sn=agTab.getForm().findField('productSnStr').getValue();
//       		 			var uuid=agTab.getForm().findField('uuid').getValue();
//       		 			var domainUuid = agTab.getForm().findField('domainUuid').getValue();
        ...
        var upgrade={ 
        		xtype:'button',
	       		...
	       		ulan:'btUpgrade',
	       		 ...
	       		 listeners:{
	       			 click:function(){
						var ids=agTab.getForm().findField('uuid').getValue();
						var maxVersion=agTab.getForm().findField('packageVersion').getValue();
						var store = agTab.store;
...
//	        				vendorId=domainInfoStore.getAt(0).get('vendorId');
		...
		if(!maintenance){
			tbs.push(commit);
			tbs.push('-');
			ip.createEditButton(agTab,agTab.store,tbs);
		...
		agTab.addDocked(dockedItems);
		
		var tab = agTab;
		var store = tab.store;
		var agLoadMask=new Ext.LoadMask(tab, {
		...
		store.on('load', function(){
			...
			var lockStatusField=tab.getForm().findField('lockStatus');
			var natModeField=tab.getForm().findField('natMode');
			var natStatusField=tab.getForm().findField('natStatus');
			var ethLinkModeField=tab.getForm().findField('ethLinkMode');
			var ntpStatusField=tab.getForm().findField('ntpStatus');
			
			lockStatusField.setValue(rs.lockStatus(r.get('lockStatus')));
			natModeField.setValue(rs.natMode(r.get('natMode')));
			natStatusField.setValue(rs.natStatus(r.get('natStatus')));
			ethLinkModeField.setValue(rs.linkMode(r.get('ethLinkMode')));
			ntpStatusField.setValue(rs.ntpStatus(r.get('ntpStatus')));
			
			removeCSSRule(styleSheet,'.memAosUsageCls::before');
			setPseudo1(styleSheet,'.memAosUsageCls:before',r.get('memAosUsage'));
			removeCSSRule(styleSheet,'.memLinuxUsageCls::before');
			setPseudo1(styleSheet,'.memLinuxUsageCls:before',r.get('memLinuxUsage'));
			removeCSSRule(styleSheet,'.storeUsageCls::before');
			setPseudo1(styleSheet,'.storeUsageCls:before',r.get('storeUsage'));
		...
		ip.initOtiose(1,agTab);
		
		var moduleId=this.moduleId;
		var agpListId=moduleId+"_agpInNe";
		var agpInNe=Ext.getCmp(agpListId);
		if(!agpInNe){
			agpInNe=Ext.create('app.view.operation.domain.roamzone.site.nes.AgPortInNe',{
				title:tiPortList,
				id:agpListId,
				moduleId:moduleId,
			});
			agpInNe.addListener("afterlayout",function(){
				privilege.procPrivilege(agpInNe);
			},this,{single:true});
		}

		
//		var agpMapId=moduleId+"_agpTab";
//		var agpTab=Ext.getCmp(agpMapId);
//		if(!agpTab){
//			agpTab=Ext.create('app.view.operation.domain.roamzone.site.nes.AgpTab',{
//				title:tiPortMap,
//				id:agpMapId,
//				moduleId:moduleId,
//				
//			});
//			agpTab.addListener("afterlayout",function(){
//				privilege.procPrivilege(agpTab);
//			},this,{single:true});
//		}
...
//				createDesc:'history',
//				nodeDesc:'ne',
//				id:'agHistoryAlarmPanel'
//			});
//			tab3.addListener("afterlayout",function(){
//				privilege.procPrivilege(tab3);
//			},this,{single:true});
			var syslogToolbars=this.syslogToolbars;
			var syslogTab = Ext.create("app.view.operation.domain.roamzone.site.nes.SyslogListPanel",{
				createDesc:'history',
				nodeDesc:'ne',
				id:'agSyslogList',
				toolbars:syslogToolbars
			});
			syslogTab.addListener("afterlayout",function(){
				privilege.procPrivilege(syslogTab);
			},this,{single:true});
			
			this.items=[{
		       	xtype: 'tabpanel',
		       	items:[agTab,agpInNe,syslogTab],
		       	listeners:{			
					tabchange:function(tabPanel,newTab,oldTab,obj){
						controller.tabpanel_tabchange(tabPanel,newTab,oldTab,obj);
					}
